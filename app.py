...
def asistencia():
    if 'usuario' not in session:
        flash('Acceso denegado.', 'danger')
        return redirect(url_for('login'))
    # ... resto del código de asistencia ...
...
def descargar_reporte_ausencias():
    ...
    conn = get_db_connection()
    try:
        # 1. Obtenemos únicamente los grados que realmente existen en tu base de datos
        ...
        for g in grados_registrados:
            # Obtener estudiantes reales de este grado
            ...
            # 2. Evaluar asistencia real para la fecha
            for est in estudiantes_grado:
                ...
                # Por defecto si no se ha pasado lista, se puede contar como presente o ausente según prefieras (aquí 'Presente')
            ...
            # Acumuladores generales reales
            total_ninos_mat += ninos_m
            total_ninas_mat += ninas_m # Nota: aquí corregimos para sumar niñas independientes
            total_mat += total_m
